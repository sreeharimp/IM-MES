...
import { LabelPaperType, ProductionPlan, ProductionPlanLabel } from '../types';
...
/**
 * Downloads valid PDF file using downloadPDFDoc.
 */
...
/**
 * Opens generated labels in a new browser tab for immediate native print.
 */
...
/**
 * Opens queue labels in new tab for immediate native print.
 */
...
/**
 * Builds calibration sheet document.
 */
export function buildCalibrationPDFDoc(paper: LabelPaperType): jsPDF {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      // Outer sticker boundary
      doc.setDrawColor(0, 0, 0);
      doc.setLineWidth(0.2);
      doc.rect(x, y, lW, lH);

      // Inner safe-print margin
