...
& "C:\Program Files\PuTTY\plink.exe" -i $ppkKey "$vpsUser@$vpsHost" "mkdir -p $remoteDir"

if ($LASTEXITCODE -ne 0) {
    Write-Host "Connection failed. If prompted for a passphrase, please enter it in the prompt." -ForegroundColor Red
    exit $LASTEXITCODE
}

# 2. Copy code and config files to the VPS using pscp
Write-Host "2. Copying project files to VPS..." -ForegroundColor Yellow
& "C:\Program Files\PuTTY\pscp.exe" -i $ppkKey -r .env Caddyfile Dockerfile .dockerignore docker-compose.yml nginx.conf package.json package-lock.json eslint.config.js tsconfig.json tsconfig.app.json tsconfig.node.json vite.config.ts index.html public src "$vpsUser@$vpsHost`:$remoteDir"
...
& "C:\Program Files\PuTTY\plink.exe" -i $ppkKey "$vpsUser@$vpsHost" "cd $remoteDir && docker compose down && docker compose up --build -d"
